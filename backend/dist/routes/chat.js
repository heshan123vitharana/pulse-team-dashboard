...
async function getTeamContext() {
    ...
    const recent_reports = await prisma_1.default.weeklyReport.findMany({
    ...
    return recent_reports.map(report => {
        ...
        if (report.blockers)
            line += `Blockers: ${report.blockers}. `;
        if (report.tasks_planned)
            line += `Next Steps: ${report.tasks_planned}.`;
        return line;
    }).join('\n');
}
// POST /api/v1/chat/message — AI chat assistant (Project Manager+)
router.post('/message', auth_1.authenticate, auth_1.requireProjectManager, async (req, res) => {
    if (!groqClient) {
        res.json({
            response: 'The Groq API key is not configured. Please add GROQ_API_KEY to your .env file to enable the AI assistant.'
        });
        return;
    }
    try {
