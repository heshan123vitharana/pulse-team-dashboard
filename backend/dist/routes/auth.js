...
router.post('/register', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    const role = await prisma_1.default.role.findUnique({ where: { id: role_id } });
    if (!role) {
...
router.post('/login', async (req, res) => {
    ...
    const access_token = jsonwebtoken_1.default.sign({ sub: user.email, role: role_name }, JWT_SECRET, { expiresIn: '24h' });
